...
**Status:** Partially Implemented — gh auth fallback (#1), check deduplication (#3), empty branch auto-fetch (#4), rebase-stack API inference (#5) done. Resolved threads subcommand (#2), Buildkite log pipeline (#6), macOS notifications (#7), CODEOWNERS/ready-unverified (#8), agent-native audit (#9) not yet implemented.
**Date:** 2026-05-06
