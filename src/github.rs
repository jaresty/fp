...
pub fn extract_session_from_cookies(cookies: &[bench_scraper::Cookie]) -> Result<String> {
    cookies.iter()
        .find(|c| c.host.contains("github.com") && c.name == "user_session")
        .map(|c| c.value.clone())
        .ok_or_else(|| anyhow::anyhow!(
            "no github.com user_session cookie found in any browser — set GITHUB_USER_SESSION or log into GitHub in a supported browser"
        ))
}

pub fn extract_github_session_from_browser() -> Result<String> {
    let all = bench_scraper::find_cookies().map_err(|e| anyhow::anyhow!("{:?}", e))?;
    let cookies: Vec<bench_scraper::Cookie> = all.into_iter()
        .flat_map(|bc| bc.cookies)
        .collect();
